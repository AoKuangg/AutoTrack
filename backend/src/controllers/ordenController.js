...
export const createOrden = async (req, res) => {
  try {
    const { id_vehiculo, diagnostico, costo_mano_obra, estado, fecha_estimada, observaciones } = req.body;
...
export const updateOrden = async (req, res) => {
  try {
    const { id } = req.params;
    const { diagnostico, costo_mano_obra, observaciones, fecha_estimada } = req.body;
...
export const updateEstadoOrden = async (req, res) => {
  try {
    ...
    // Validar estado
    const estadosValidos = ['diagnostico', 'reparando', 'finalizado', 'entregado', 'cancelado'];
    if (!estadosValidos.includes(estado)) {
      return res.status(400).json({ 
        error: 'Estado inválido. Debe ser: diagnostico, reparando, finalizado, entregado o cancelado' 
      });
    }
    ...
    const result = await pool.query(
      `UPDATE orden_servicio SET ${updates.join(', ')} WHERE id_orden = $${values.length} RETURNING *`,
      values
    );
...
export const addRepuestoOrden = async (req, res) => {
  try {
    ...
    const costoTotal = parseFloat(total.rows[0].total_repuestos) + parseFloat(total.rows[0].costo_mano_obra);
...
export const removeRepuestoOrden = async (req, res) => {
  try {
    ...
    if (uso.rows.length === 0) {
      return res.status(404).json({ error: 'Repuesto no encontrado en esta orden' });
    }
    ...
    if (total.rows.length > 0) {
      costoTotal = parseFloat(total.rows[0].total_repuestos) + parseFloat(total.rows[0].costo_mano_obra);
    } else {
      const orden = await pool.query('SELECT costo_mano_obra FROM orden_servicio WHERE id_orden = $1', [id]);
      costoTotal = parseFloat(orden.rows[0].costo_mano_obra);
    }

    await pool.query('UPDATE orden_servicio SET costo_total = $1 WHERE id_orden = $2', [costoTotal, id]);

    res.json({ message: 'Repuesto eliminado exitosamente', costo_total: costoTotal });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
};